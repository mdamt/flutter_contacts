package flutter.plugins.ngobrel.contacts;
...
public class ContactsPlugin implements MethodCallHandler {

  ContactsPlugin(ContentResolver contentResolver) {
  ...
  public static void registerWith(Registrar registrar) {
    final MethodChannel channel = new MethodChannel(registrar.messenger(), "github.com/ngobrel/contacts");
    channel.setMethodCallHandler(new ContactsPlugin(registrar.context().getContentResolver()));
  }
  ...
  public void onMethodCall(MethodCall call, Result result) {
    switch (call.method) {
      ...
      case "getContactsByPhoneNumber":
        ...
        this.getContactsByPhoneNumber(args.get(0), args.get(1), result);
        break;
      case "addContact":
  ...
  @TargetApi(Build.VERSION_CODES.ECLAIR)
  private void getContactsByPhoneNumber(String query, String countryPrefix, Result result) {
    new GetContactsTaskByPhoneNumber(result).execute(new String[]{query, countryPrefix});
  }

  @TargetApi(Build.VERSION_CODES.CUPCAKE)
  private class GetContactsTaskByPhoneNumber extends AsyncTask<String, Void, ArrayList<HashMap>> {

    private Result getContactResult;

    public GetContactsTaskByPhoneNumber(Result result) {
      this.getContactResult = result;
    }

    @TargetApi(Build.VERSION_CODES.ECLAIR)
    protected ArrayList<HashMap> doInBackground(String... query) {
      ArrayList<Contact> contacts = getContactsFrom(getCursorByPhoneNumber(query[0], query[1]));
  ...
  private Cursor getCursorByPhoneNumber(String query, String countryPrefix) {
    String prefix = countryPrefix;
    if (prefix == null) {
      prefix = "";
    }
    String selection = String.format("'%s'||replace(ltrim(replace(replace(%s, ' ',''), '-', ''),'0'),'%s','') like ? AND %s=?",
        prefix, Phone.NUMBER, prefix, ContactsContract.Data.MIMETYPE);

    System.out.println(prefix +":"+ selection);
    String[] selectionArgs = new String[]{ query, Phone.CONTENT_ITEM_TYPE};
